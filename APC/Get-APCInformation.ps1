...
DESCRIPTION:
Parses Config Files from "Get-APCConfig.ps1", or downloaded manually, for information about the APC

USAGE:
.\Get-APCInformation.ps1 -APCConfigDir 'Directory' -ReportPath 'Path to save CSV'
...
param (
    [Parameter(Mandatory = $true)]$APCConfigDir,
    [Parameter(Mandatory = $true)]$ReportPath
)

$DirList = Get-ChildItem -Path $APCConfigDir
$DirList = $DirList | Where-Object {$_.extension -eq ".ini"}
$APCConfigFiles = $DirList.VersionInfo.FileName
